...
git() {
  ...
  if [[ "$1" == "push" && "$2" == "origin" && "$3" == "HEAD" && $# -eq 3 ]]; then
    echo
    echo -e "${BOLD}${RED}████████████████████████████████████████████████████████████${RESET}"
    echo -e "${RED}❌  STOP! Direct HEAD push to origin is blocked!           ❌${RESET}"
    echo -e "${GREEN}👉  Use: git push origin HEAD:refs/for/master${RESET}"
    echo -e "${BOLD}${RED}████████████████████████████████████████████████████████████${RESET}"
    echo
    return 1
  fi

  # otherwise, call the real git
  command git "$@"
}
...
# fnm
FNM_PATH="/home/amourabe/.local/share/fnm"
if [ -d "$FNM_PATH" ]; then
